...
def load_rdf_data():
    ...
    try:
        g1 = Graph()
        g1.parse("/Users/yingji/Library/CloudStorage/OneDrive-MicrosoftOffice365/VU_course/AI_master/ML for Graph/RDF2Vec/dataset/AIFB.n3", format="n3")
        graphs.append(g1)
    except Exception as e:
        print(f"Error parsing RDF files: {e}")
    return graphs

# 2. Generate Graph Walks (BFS)
...
def train_word2vec(walks, dimensions=200, window_size=5, sg=0, negative=25, epochs=1):
    model = Word2Vec(sentences=walks, vector_size=dimensions, window=window_size, sg=sg, negative=negative, epochs=epochs, workers=4)
    for _ in range(10):
...
for model_type, algorithm, dimensions, sg in models:
    ...
    for train_index, test_index in kf.split(sampled_triples):
        ...
        filtered_affiliations = [(x, y) for x, y in zip(X_train_nodes, Y_train_nodes) if x in embedding_model.wv and y in embedding_model.wv]
        ...
        filtered_test_affiliations = [(x, y) for x, y in zip(X_test_nodes, Y_test_nodes) if x in embedding_model.wv and y in embedding_model.wv]
